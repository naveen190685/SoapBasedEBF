...
public class ReportLibrary extends ExcelFileUtil {
	...
	public void sendLog(String Trace, String testCaseName, int testStep) {
		try {
			// Create file
...
//						+ "<td>" + StartTime + "</td>" + "<td>" + strEndTime + "</td>" + failTxt + "<td>" + failedStep
	...
	public void f_sendTestStepResult(String TCID, String strModulename, String strTestcasename, String strTestStepID,
			String strScreen, String strActionName, String strFieldname, String strValue, int intStatus)
			throws Exception {
		try {
			...
			if (intStatus == 0) {
				...
				FileUtils.copyFile(screenShoot, file);
				...
				out_detailedResult.write("<tr style=" + "\"" + "color:Red" + "\"" + ">" + "<td>" + strModulename
						+ "</td>" + "<td>" + TCID + "</td><td>" + strTestcasename + "</td><td>" + strTestStepID
	...
	public void zipReportsAndSendMail(FunctionalLibrary seleniumHandler) throws Exception {
		...
		System.out.println("Total Testcases Executed: " + ReportLibrary.totalTCount);
		...
		if (sendMail.equalsIgnoreCase("YES")) {
			if (mailinglist.isEmpty() || mailinglist.equals(" ")) {
				...
				String args = zippath + " " + ReportLibrary.totalTCount + " " + passedTCount + " " + failedTCount
